...
class MIRT(CDM):
    def __init__(self, user_num, item_num, latent_dim, a_range=None, theta_range=None):
        super(MIRT, self).__init__()
        self.irt_net = MIRTNet(user_num, item_num, latent_dim, a_range, theta_range)

    def train(self, train_data, test_data=None, *, epoch: int, device="cpu", lr=0.001, cc=0.8) -> ...:
        ...
        for e in range(epoch):
            ...
            for batch_data in tqdm(train_data, "Epoch %s" % e):
                user_id, item_id, response, group = batch_data
                ...
                loss = loss_function(predicted_response, response)
                group_diff = torch.abs(torch.abs(predicted_response[group0].mean()-response[group0].mean()) - torch.abs(predicted_response[group1].mean()-response[group1].mean()))
                loss = loss + cc * group_diff
