...
class NCDM(CDM):
    ...
    def train(self, train_data, test_data=None, epoch=10, device="cpu", lr=0.002, silence=False, cc=0.8):
        ...
        for epoch_i in range(epoch):
            ...
            for batch_data in tqdm(train_data, "Epoch %s" % epoch_i):
                batch_count += 1
                user_id, item_id, knowledge_emb, y, group = batch_data
                ...
                loss = loss_function(pred, y)
                group_diff = torch.abs(torch.abs(pred[group0].mean()-y[group0].mean()) - torch.abs(pred[group1].mean()-y[group1].mean()))
